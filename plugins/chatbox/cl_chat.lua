-- impulse's chatbox is based upon impulse.chatBox by Exho
...
-- Define a new function to draw the watermark
local function DrawWatermark(frame)
    local text = "HL2.Land SeriousRP"
    local font = "Impulse-ChatSmall"
    local color = color_white

    surface.SetFont(font)
    local textWidth, textHeight = surface.GetTextSize(text)

    local xPos = frame:GetWide() - textWidth - 10
    local yPos = 5 -- Modified to drop down about 30 pixels

    -- Draw the outlined text
    draw.SimpleTextOutlined(text, font, xPos, yPos, color, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, 255))
end
...
function impulse.chatBox.buildBox()
    ...
    impulse.chatBox.frame.Paint = function(self, w, h)
        ...
        -- Call the watermark drawing function
        DrawWatermark(self)
    end
...
hook.Add("PlayerBindPress", "impulse.chatBox_hijackbind", function(ply, bind, pressed)
    if string.sub(bind, 1, 11) == "messagemode" then
        if ply:InVehicle() then -- piano compatibility kill me
