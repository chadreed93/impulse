-- impulseCraftingmenu shit reworked to fit the inventory so ppl dont always need a BIG FUCKING TABLE to craft a BANDAGE

local PANEL = {}

function PANEL:Init()
	self:Dock(FILL)
  -- sdlfkjdslkfjsdlkfjsdlkjflsdkjflkdsjflk help i am tire
end
...
function PANEL:SetupCrafting()
	...
	local benchClass = impulse.Inventory.Benches["hands"] or {}
	local panel = self
	...
	local skill = benchClass.Skill or "craft"
	self.craftLbl:SetText(impulse.Skills.GetNiceName(skill).." Level: "..LocalPlayer():GetSkillLevel(skill))
	...
	local level = LocalPlayer():GetSkillLevel(skill)
	local mix = impulse.Inventory.Mixtures[benchType] or {}
	local sortedMix = {}
...
vgui.Register("impulseCraftingMenuInset", PANEL, "DPanel")
