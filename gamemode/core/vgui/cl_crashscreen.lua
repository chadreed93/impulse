...
local waits = {}
local watermarkMaterial = Material("hl2land/Smaller.png")
...
function PANEL:DoLamar()
    ...
    local function doPostText()
        ...
        self.backsoon:SetText("Reconnecting you now!")
...
function PANEL:PaintOver()
    ...
    local h = ScrH() * 0.3
    w = 20
    h = 30 + 50 -- Adjusted by 50 pixels

    -- Draw the image above the connection lost text
    local imgWidth, imgHeight = 256, 64 -- Adjust these values based on your image dimensions
    local imgX, imgY = (ScrW() - imgWidth) / 2, h - imgHeight - 20 -- Centered horizontally and positioned above the text

    surface.SetMaterial(watermarkMaterial)
    ...
    if self.ServerIsOff then
        draw.SimpleText("The server has gone offline. It will try to reconnect in a minute...", "Impulse-Elements22-Shadow", w, h + 70)
        draw.SimpleText("You will be refunded automatically.", "Impulse-Elements19-Shadow", w, h + 90)
    else
        draw.SimpleText("You've lost connection to the server. It will try to reconnect in a minute...", "Impulse-Elements22-Shadow", w, h + 70)
        draw.SimpleText("Check your router or internet connection if you still have issues.", "Impulse-Elements19-Shadow", w, h + 90)
    end
end
