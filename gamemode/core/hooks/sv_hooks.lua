...
/* Shit be not working right, lets just focus on getting the server up and do this side quest bullshit later. 
function GM:EntityTakeDamage(target, dmginfo)
    if target:IsPlayer() then
        local hitgroup = target:LastHitGroup()

        if hitgroup == HITGROUP_HEAD then
            -- Potentially handle head injuries here
        elseif hitgroup == HITGROUP_CHEST then
            if math.random(1, 100) <= impulse.Config.BrokenRibsChance then
                target:BreakRibs()
                target:Notify("Your ribs are broken!")
            end
        elseif hitgroup == HITGROUP_STOMACH then
            if math.random(1, 100) <= impulse.Config.CriticalBleedingChance then
                target:StartBleeding()
                target:Notify("You are bleeding critically!")
            end
        elseif hitgroup == HITGROUP_LEFTARM or hitgroup == HITGROUP_RIGHTARM then
            if math.random(1, 100) <= impulse.Config.BrokenArmsChance then
                target:BreakArms()
                target:Notify("Your arms are broken!")
            end
        elseif hitgroup == HITGROUP_LEFTLEG or hitgroup == HITGROUP_RIGHTLEG then
            if math.random(1, 100) <= impulse.Config.BrokenLegsChance then
                target:BreakLegs()
                target:Notify("Your legs are broken!")
            end
        end
    end
end
...
-- Register the hook
hook.Add("EntityTakeDamage", "HandlePlayerInjuries", function(target, dmginfo)
    GM:EntityTakeDamage(target, dmginfo)
end)
