...
local bloodVignetteMaterial = Material("hl2land/blood.png")
local redVignetteMaterial = Material("cn-hl2rp/impulse/vignette.png")
...
function GM:RenderScreenspaceEffects()
    ...
    -- Darken the screen and add red tint as health drops below 80
    if health < 80 then
        local effectIntensity = math.Clamp((80 - health) / 80, 0, 1)
        local colorModify = {
            ["$pp_colour_addr"] = effectIntensity * 0.4,
            ["$pp_colour_addg"] = effectIntensity * 0.1,
            ["$pp_colour_addb"] = effectIntensity * 0.1,
            ["$pp_colour_brightness"] = -effectIntensity * 0.1,
            ["$pp_colour_contrast"] = 1,
            ["$pp_colour_colour"] = 1 - (effectIntensity * 0.8),
            ["$pp_colour_mulr"] = 0,
            ["$pp_colour_mulg"] = 0,
            ["$pp_colour_mulb"] = 0
        }
        ...
        local redAlpha = effectIntensity
        surface.SetDrawColor(255, 0, 0, redAlpha * 255)
        surface.SetMaterial(bloodVignetteMaterial)
        surface.DrawTexturedRect(0, 0, ScrW(), ScrH())
    end

    -- Add pulsing red vignette as health drops below 50
    if health <= 50 then
        local pulseSpeed = 2 + (50 - health) * 0.1
        local pulse = math.abs(math.sin(CurTime() * pulseSpeed)) * 255
        surface.SetDrawColor(255, 0, 0, pulse)
        surface.SetMaterial(redVignetteMaterial)
        surface.DrawTexturedRect(0, 0, ScrW(), ScrH())
    end
end
