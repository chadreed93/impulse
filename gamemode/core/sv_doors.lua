    ...
    for _, k in pairs(ents.GetAll()) do
        if k:IsDoor() and k:CreatedByMap() then
            doors[k:MapCreationID()] = {
                ...
                pos = k:GetPos(),
                buyable = k:GetSyncVar(SYNC_DOOR_BUYABLE, true)
            }
        end
    end

    local doorCount = table.Count(doors)  -- Get the count of elements in the table
    print("[impulse] Saving doors to impulse/doors/"..game.GetMap()..".dat | Doors saved: "..doorCount)
...
function impulse.Doors.Load()
    ...
    if file.Exists(fileName..".dat", "DATA") then
        ...
        -- use position hashes so we don't take several seconds
        ...
        for _, k in pairs(ents.GetAll()) do
            ...
            if found and k:IsDoor() then
                ...
                if doorData.buyable ~= nil then doorEnt:SetSyncVar(SYNC_DOOR_BUYABLE, doorData.buyable, true) end
            end
        end

        -- and doors we couldn't get by pos, we'll fallback to hammerID's (less update safe) (old method)
        for doorID, doorData in pairs(mapDoorData) do
